...
function matches(employe: Employee, term: string, pipe: PipeTransform) {
  return (
    employe.username.toLowerCase().includes(term.toLowerCase()) ||
    pipe.transform(employe.identifiant).includes(term) ||
    pipe.transform(employe.email).includes(term)
  );
}
